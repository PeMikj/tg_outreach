...
- `sqlite` как локальный persistent store и DB-backed queue
...
| Primary database | SQLite with WAL / busy timeout | Достаточно для single-node PoC, минимум operational overhead |
...
14. Execution result, agent trace, метрики и audit events записываются в observability stack.
